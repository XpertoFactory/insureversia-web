...
src/
  ...
  lib/            # Shared utilities (firebase.ts, auth.ts, tiers.ts, conversations.ts, chat.ts, chat-persona.ts, chat-suggestions.ts, newsletter.ts, submissions.ts)
...
- Build must pass (`npm run build`) before committing — expect 62 pages, zero errors
