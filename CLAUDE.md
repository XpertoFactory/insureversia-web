...
src/
  ...
  lib/            # Shared utilities (firebase.ts, auth.ts, tiers.ts, conversations.ts, chat.ts, chat-persona.ts, chat-suggestions.ts, newsletter.ts, submissions.ts, comments.ts, admin.ts, likes.ts)
  pages/          # File-based routing (31 EN pages + 1 admin page)
  pages/es/       # Spanish mirror pages (31 ES pages)
...
- **Firestore collections:** `likes/`, `submissions/`, `users/` (usage tracking), `conversations/` (chat history with `messages/` subcollection), `newsletters/` (email subscriptions), `comments/` (moderated user comments)
...
- Build must pass (`npm run build`) before committing — expect 63 pages, zero errors
